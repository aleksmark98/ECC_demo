...
#encrypt and decrypt all points
...
for point in Alice.pts:
    ...
    C1, C2 = Alice.encrypt(message, Bob.public_Q)
    decrypted_message = Bob.decrypt(C1, C2)
    successful_decryption = all(message == decrypted_message)
    if (not successful_decryption):
...
print(" is encrypted and decrypted string the same: ", string_message == decrypted_string)
